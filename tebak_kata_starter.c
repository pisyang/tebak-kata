...
void play_game(void) {
    ...
    */
    ...
    /* baris di bawah hanya agar kerangka tidak memunculkan warning;
       hapus jika sudah tidak diperlukan */
    (void) g; (void) name; (void) idx; (void) len; (void) i;
}
...
void insert_word_sorted(const char *bank[], int *n, const char *new_word) {
    /* TODO: alokasi salinan kata, geser elemen yang lebih besar ke kanan,
             sisipkan, lalu (*n)++ */
    (void) bank; (void) n; (void) new_word;
}
